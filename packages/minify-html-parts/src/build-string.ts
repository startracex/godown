...
export const buildString = (
  ...
  const head = src.slice(templateExpressionResult.start, templateExpressionResult.template.getStart());
  const tail = src.slice(templateExpressionResult.template.getEnd(), templateExpressionResult.end);
